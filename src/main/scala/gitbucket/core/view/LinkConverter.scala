...
trait LinkConverter { self: RequestCache =>
  ...
  protected def convertRefsLinks(value: String, repository: RepositoryService.RepositoryInfo,
      issueIdPrefix: String =  "#", escapeHtml: Boolean = true)(implicit context: LinkContext): String = {
    ...
    escaped
      ...
      .replaceBy(("(?<=(^|\\W))" + issueIdPrefix + "([0-9]+)(?=(\\W|$))").r){ m =>
        ...
        getIssue(repository.owner, repository.name, m.group(2)) match {
          ...
          case Some(_) => Some(s"""<a href="${context.baseUrl}/${repository.owner}/${repository.name}/issues/${m.group(2)}">#${m.group(2)}</a>""")
          case None    => Some(s"""#${m.group(2)}""")
        }
      }
      // convert @username to link
      .replaceBy("(?<=(^|\\W))@([a-zA-Z0-9\\-_\\.]+)(?=(\\W|$))".r){ m =>
        getCachedAccountByUserName(m.group(2)).map { _ =>
          s"""<a href="${context.baseUrl}/${m.group(2)}">@${m.group(2)}</a>"""
        }
      }
      // convert commit id to link
      .replaceAll("(?<=(^|\\W))([a-f0-9]{40})(?=(\\W|$))", s"""<a href="${context.baseUrl}/${repository.owner}/${repository.name}/commit/$$2">$$2</a>""")
  }
}
